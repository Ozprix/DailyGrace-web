...
export class EmailService {
  ...
  public async sendWeeklyDigests(): Promise<any> {
    ...
    for (const user of users) {
      ...
      functions.logger.info(`--- Weekly Digest for ${user.userId} ---`);
      functions.logger.info(emailHtml);
      functions.logger.info(`--- End Digest ---`);
      sentCount++;
    }

    return { success: true, message: `Successfully generated ${sentCount} digest(s).` };
  }
}
